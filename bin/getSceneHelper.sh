...
case $1 in
start)
    PROC_ID=`ps -ef|grep "$APP_PATHNAME $3 $4 $5 $6 $7 $8 $9"|grep -v grep|awk '{print $2}'`
    if [ X"$PROC_ID" != X ]
    then
...
daemon)
    ...
    then
        ...
        if [ -e $APP_PATHNAME ] ; then
            $APP_PATHNAME  $3 $4 $5 $6 $7 $8 $9 > ./log/getscenelog.log 2>&1 &
        else
            echo "$APP_PATHNAME is not in current directory,Please change to correct directory"
            exit 2
        fi
...
stop)
    ...
    ;;    
status)
    RET=`ps -ef|grep "$APP_PATHNAME  $3 $4 $5 $6 $7 $8 $9"|grep -v grep`
    if [ X"$RET" = X ]
    then
        exit -1
    fi
