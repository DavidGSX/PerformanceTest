#!/bin/bash
##############################################################
# SprRobotRunHelper.sh V1.0.1                                  #
#------------------------------------------------------------#
# History:                                                   #
#  1.2017/11/23 create                                       #
#------------------------------------------------------------#
# Author: jennachen Date:2018/05/18                         #
# CopyRight (C) 2017 by Tencent                          #
##############################################################
readonly APP_PATHNAME=./$2
ROBOT_ARGS=""
ARGS_INDEX=1
for i in $@
do
...
case $1 in
start)
    PROC_ID=`ps -ef|grep "$APP_PATHNAME $ROBOT_ARGS"|grep -v grep|awk '{print $2}'`
    if [ X"$PROC_ID" != X ]
    then
...
daemon)
    ...
    then
        ...
        if [ -e $APP_PATHNAME ] ; then
            $APP_PATHNAME $ROBOT_ARGS > ./log/sprrobotrun.log 2>&1 &
		else
            echo "$APP_PATHNAME is not in current directory,Please change to correct directory"
            exit 2
        fi
...
stop)
    ...
    ;;    
status)
    RET=`ps -ef|grep "$APP_PATHNAME $ROBOT_ARGS"|grep -v grep`
    if [ X"$RET" = X ]
    then
        exit -1
    fi
